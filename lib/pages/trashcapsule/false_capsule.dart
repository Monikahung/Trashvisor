...
class _ImpactCard extends StatelessWidget {
  ...
  const _ImpactCard({required this.title, required this.description, this.imageUrl, this.fallbackAsset});

  @override
  Widget build(BuildContext context) {
    Widget image;
    if (imageUrl != null && imageUrl!.isNotEmpty) {
      image = Image.network(imageUrl!, width: 90, height: 110, fit: BoxFit.cover,
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(title, style: const TextStyle(
              fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.darkMossGreen, fontFamily: 'Nunito',
            )),
            const SizedBox(height: 4),
            Text(description, style: const TextStyle(fontSize: 14, color: Colors.black, fontFamily: 'Roboto')),
          ]),
        ),
      ]),
    );
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(label, style: const TextStyle(
            fontSize: 16, color: AppColors.whiteSmoke, fontWeight: FontWeight.bold, fontFamily: 'Nunito',
          )),
        ]),
      ),
    );
...
class FalseTrashCapsule extends StatefulWidget {
  ...
  const FalseTrashCapsule({super.key, required this.cameras, this.initialWasteType});
...
class _FalseTrashCapsuleState extends State<FalseTrashCapsule> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        toolbarHeight: 80, backgroundColor: AppColors.mossGreen, elevation: 0,
        leading: IconButton(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
            ...
            const Padding(
              padding: EdgeInsets.symmetric(horizontal: 24.0),
              child: Text('Penanganan sampah yang buruk akan berakibat fatal bagi masa depan bumi.',
                  style: TextStyle(fontSize: 14, color: Colors.black, fontFamily: 'Roboto')),
            ),
            const SizedBox(height: 24),
